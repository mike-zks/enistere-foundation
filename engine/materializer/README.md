...
| `materialize.ts` | `planMaterialization` (sans I/O) et `materialize` : écrit `<workspace>/<composant>/` selon `decideWrite`, et enregistre un **MaterializationRecord (A8)** par composant dans `.foundation/records/` (révision n+1, la précédente passe SUPERSEDED) ; le dernier A8 est l'unique inventaire de propriété ; un composant en conflit n'est pas écrit (record CONFLICT retourné, non stocké) ; instant `executedAt` injecté |
