...
export const DIAGNOSTIC_CODES = Object.freeze({
  ...
  SYSTEM_UNKNOWN_DESIGN_CONTEXT: spec('INVALID_INPUT', 'error', 'Bind the surface to a design context declared by the pinned Design System.'),
  DESIGN_TOKEN_INVALID_VALUE: spec('INVALID_INPUT', 'error', 'Give the token a value of its $type (color #RRGGBB, dimension px/rem, duration ms/s, numbers…) or an alias.'),
  DESIGN_TOKEN_UNRESOLVED_ALIAS: spec('INVALID_INPUT', 'error', 'An alias {group.token} must name a token of the design system.'),
  DESIGN_TOKEN_TYPE_MISMATCH: spec('INVALID_INPUT', 'error', 'An alias must name a token of the same $type.'),
  DESIGN_TOKEN_ALIAS_CYCLE: spec('INVALID_INPUT', 'error', 'Break the alias cycle: every alias must end on a literal value.'),
  DESIGN_CONTEXT_UNKNOWN_TOKEN: spec('INVALID_INPUT', 'error', 'A design context can only override tokens the design system declares.'),
  POLICY_VIOLATION: spec('POLICY_DENIED', 'error', 'Change the System Definition to comply, or obtain a human-approved waiver in a new revision of the organization context.'),
  POLICY_NOT_EVALUATED: spec('UNSUPPORTED', 'warning', 'The compiler does not interpret this rule: prove it by another check or review it.'),
  MATERIALIZE_WAIVER_EXPIRED: spec('POLICY_DENIED', 'error', 'The plan relies on an expired waiver: renew it in a new revision of the organization context, or comply.'),
