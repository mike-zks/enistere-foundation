...
test('every diagnostic code maps to a failure class of document 03 and has a remediation', () => {
  for (const [code, spec] of Object.entries(DIAGNOSTIC_CODES)) {
    ...
    assert.match(code, /^(CONTRACT|AUTHORITY|REF|REQUIREMENT|DECISION|CONTEXT|SYSTEM|DOMAIN|CHANGE|EVIDENCE|CATALOG|RESOLVE|FACADE|MANIFEST|ADAPTER|MATERIALIZE|VERIFY|IR)_[A-Z_]+$/);
  }
