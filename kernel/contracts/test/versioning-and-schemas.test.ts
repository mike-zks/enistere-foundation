...
test('the registry covers A1–A7 exactly once with Desired / Derived / Evidence planes', () => {
  const entries = Object.values(CONTRACT_REGISTRY);
  assert.deepEqual(entries.map((entry) => entry.code).sort(), ['A1', 'A2', 'A3', 'A4', 'A5', 'A6', 'A7']);
