...
> Dernière mise à jour : 2026-09-26, fin de la mission E4 (A8 MaterializationRecord et proof chain, ADR-098).
...
|---|---|
| Foundation Kernel | [`kernel/contracts/`](kernel/contracts/README.md) — contrats A1–A7 (E0) et A8 MaterializationRecord (E4) ; [`kernel/compiler/`](kernel/compiler/README.md) — proof chain v1 (E4) ; [`kernel/compiler/`](kernel/compiler/README.md) — Kernel Façade et chaîne closure → IR (avec Domain IR, E3) → résolution → plan (E1) ; [`kernel/extensions/`](kernel/extensions/README.md) — Adapter Protocol v0 (E2) |
...
du dossier supposant une couche de compatibilité) ADR-095 (E1, chaîne de compilation native), ADR-096 (E2, Adapter Protocol v0) ADR-097 (E3, Domain IR) et ADR-098 (E4, A8 et proof chain).

## Mission

- **Missions terminées** : E0 — Contract Foundation (PASS, P1–P10 validés), R0-C — Repository
  Realignment (ADR-093), R0-D — repartir propre (ADR-094) et E1 — Kernel Façade (ADR-095) E2 — Adapter Protocol v0 (ADR-096) E3 — Domain IR (ADR-097) et E4 — Ownership & Evidence (ADR-098). Résultats :
  [`CURRENT_STATE.md`](CURRENT_STATE.md).
- **Périmètre retenu** : greenfield, modèle de contrats ; brownfield et lifecycle complet hors périmètre.
- **Prochaine mission unique** : **E5 — Domain Contract Projection** (horizon V1, [`BACKLOG.md`](BACKLOG.md)).
